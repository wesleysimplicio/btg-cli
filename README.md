...
CLI para BTG Pactual — investimentos, contas e serviços BTG
...
2. Use a skill **`btg-br`** para entender o vocabulário e os fluxos esperados.
...
- **Nome do pacote:** `btg-cli`
- **Comando instalado:** `btg`
- **Runtime:** Python 3.9+
- **API base:** `https://api.btgpactual.com/`
- **Documentação da API:** https://developers.btgpactual.com/
- **Endpoints mapeados:** variam por serviço
- **Auth:** OAuth 2.0 / Client Credentials (padrão Open Finance / bancos)
- **Cache local de sessão:** `~/.hermes2/scripts/btg-cli/`
...
1. Leia `AGENTS.md` antes de editar qualquer coisa.
2. Use a skill **`btg-br`** para entender os fluxos esperados.
...
```bash
git clone https://github.com/wesleysimplicio/btg-cli.git
cd btg-cli
pip install -e .
```
...
| Comando | O que faz |
|---|---|
| `btg --help` | lista os comandos disponíveis |
| `btg config set` | define base URLs, certificados e credenciais locais |
| `btg config show` | mostra a configuração salva |
| `btg config path` | mostra o arquivo de configuração usado pelo CLI |
...
A skill `btg-br` permite que o Hermes execute ações como:
...
```bash
claude -p "Rode btg --help e me diga o que está disponível"
codex -p "Rode btg config show e resuma a configuração"
```
